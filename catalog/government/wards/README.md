# Ward Boundaries (2020)

Boundaries of City of St. Louis wards Mirrored from [the city's open data portal](https://www.stlouis-mo.gov/data/datasets/dataset.cfm?id=131); explore it in the [St. Louis data browser](https://cholmes.github.io/stlouis-data-browser/#/government/wards/collection.json).

![st-louis](https://img.shields.io/badge/st--louis-blue) ![missouri](https://img.shields.io/badge/missouri-blue) ![open-data](https://img.shields.io/badge/open--data-blue) ![wards](https://img.shields.io/badge/wards-blue)

## Spatial Coverage

- **Bounding Box**: [-90.32051499970312, 38.53201199995981, -90.16627099969693, 38.774368000291375]

## Temporal Coverage

- **Start**: open
- **End**: ongoing

## Schema

| Column | Type | Description |
|--------|------|-------------|
| fid | int64 |  |
| id | int32 |  |
| stlouis_gi | double |  |
| district | string |  |
| members | double |  |
| locked | string |  |
| name | string |  |
| population | int32 |  |
| white | int32 |  |
| black | int32 |  |
| amindian | int32 |  |
| asian | int32 |  |
| hawaiian | int32 |  |
| hispanic_o | int32 |  |
| not_hispan | int32 |  |
| other | int32 |  |
| f18_pop | int32 |  |
| f18_wht | int32 |  |
| f18_blk | int32 |  |
| f18_ind | int32 |  |
| f18_asn | int32 |  |
| f18_hwn | int32 |  |
| f18_oth | int32 |  |
| nh18_pop | int32 |  |
| nh18_wht | int32 |  |
| nh18_blk | int32 |  |
| nh18_ind | int32 |  |
| nh18_asn | int32 |  |
| nh18_hwn | int32 |  |
| nh18_oth | int32 |  |
| whtblk | int32 |  |
| whtind | int32 |  |
| whtasn | int32 |  |
| whthwn | int32 |  |
| whtoth | int32 |  |
| blkind | int32 |  |
| blkasn | int32 |  |
| blkhwn | int32 |  |
| blkoth | int32 |  |
| indasn | int32 |  |
| indhwn | int32 |  |
| indoth | int32 |  |
| asnhwn | int32 |  |
| asnoth | int32 |  |
| hwnoth | int32 |  |
| whtblkind | int32 |  |
| whtblkasn | int32 |  |
| whtblkhwn | int32 |  |
| whtblkoth | int32 |  |
| whtindasn | int32 |  |
| whtindhwn | int32 |  |
| whtindoth | int32 |  |
| whtasnhwn | int32 |  |
| whtasnoth | int32 |  |
| whthwnoth | int32 |  |
| blkindasn | int32 |  |
| blkindhwn | int32 |  |
| blkindoth | int32 |  |
| blkasnhwn | int32 |  |
| blkasnoth | int32 |  |
| blkhwnoth | int32 |  |
| indasnhwn | int32 |  |
| indasnoth | int32 |  |
| indhwnoth | int32 |  |
| asnhwnoth | int32 |  |
| whtblkinda | int32 |  |
| whtblkindh | int32 |  |
| whtblkindo | int32 |  |
| whtblkasnh | int32 |  |
| whtblkasno | int32 |  |
| whtblkhwno | int32 |  |
| whtindasnh | int32 |  |
| whtindasno | int32 |  |
| whtindhwno | int32 |  |
| whtasnhwno | int32 |  |
| blkindasnh | int32 |  |
| blkindasno | int32 |  |
| blkindhwno | int32 |  |
| blkasnhwno | int32 |  |
| indasnhwno | int32 |  |
| whtblkind1 | int32 |  |
| whtblkind2 | int32 |  |
| whtblkind3 | int32 |  |
| whtblkasn1 | int32 |  |
| whtindasn1 | int32 |  |
| blkindasn1 | int32 |  |
| all_races | int32 |  |
| nh_asn | int32 |  |
| ap_wht | int32 |  |
| np_wht | int32 |  |
| ap_blk | int32 |  |
| np_blk | int32 |  |
| ap_ind | int32 |  |
| np_ind | int32 |  |
| ap_asn | int32 |  |
| np_asn | int32 |  |
| ap_hwn | int32 |  |
| np_hwn | int32 |  |
| ap_oth | int32 |  |
| np_oth | int32 |  |
| f1_race | int32 |  |
| f2_races | int32 |  |
| f2_races1 | int32 |  |
| f3_races | int32 |  |
| f4_races | int32 |  |
| f5_races | int32 |  |
| omb_othmr | int32 |  |
| nh_wht | int32 |  |
| nh_blk | int32 |  |
| nh_ind | int32 |  |
| nh_hwn | int32 |  |
| nh_oth | int32 |  |
| nh_whtblk | int32 |  |
| nh_whtind | int32 |  |
| nh_whtasn | int32 |  |
| nh_whthwn | int32 |  |
| nh_whtoth | int32 |  |
| nh_blkind | int32 |  |
| nh_blkasn | int32 |  |
| nh_blkhwn | int32 |  |
| nh_blkoth | int32 |  |
| nh_indasn | int32 |  |
| nh_indhwn | int32 |  |
| nh_indoth | int32 |  |
| nh_asnhwn | int32 |  |
| nh_asnoth | int32 |  |
| nh_hwnoth | int32 |  |
| nh_whtblki | int32 |  |
| nh_whtblka | int32 |  |
| nh_whtblkh | int32 |  |
| nh_whtblko | int32 |  |
| nh_whtinda | int32 |  |
| nh_whtindh | int32 |  |
| nh_whtindo | int32 |  |
| nh_whtasnh | int32 |  |
| nh_whtasno | int32 |  |
| nh_whthwno | int32 |  |
| nh_blkinda | int32 |  |
| nh_blkindh | int32 |  |
| nh_blkindo | int32 |  |
| nh_blkasnh | int32 |  |
| nh_blkasno | int32 |  |
| nh_blkhwno | int32 |  |
| nh_indasnh | int32 |  |
| nh_indasno | int32 |  |
| nh_indhwno | int32 |  |
| nh_asnhwno | int32 |  |
| nh_whtblk1 | int32 |  |
| nh_whtblk2 | int32 |  |
| nh_whtblk3 | int32 |  |
| nh_whtblk4 | int32 |  |
| nh_whtblk5 | int32 |  |
| nh_whtblk6 | int32 |  |
| nh_whtind1 | int32 |  |
| nh_whtind2 | int32 |  |
| nh_whtind3 | int32 |  |
| nh_whtasn1 | int32 |  |
| nh_blkind1 | int32 |  |
| nh_blkind2 | int32 |  |
| nh_blkind3 | int32 |  |
| nh_blkasn1 | int32 |  |
| nh_indasn1 | int32 |  |
| nh_whtblk7 | int32 |  |
| nh_whtblk8 | int32 |  |
| nh_whtblk9 | int32 |  |
| nh_whtbl10 | int32 |  |
| nh_whtind4 | int32 |  |
| nh_blkind4 | int32 |  |
| nh_all_rac | int32 |  |
| nh_ap_wht | int32 |  |
| nh_np_wht | int32 |  |
| nh_ap_blk | int32 |  |
| nh_np_blk | int32 |  |
| nh_ap_ind | int32 |  |
| nh_np_ind | int32 |  |
| nh_ap_asn | int32 |  |
| nh_np_asn | int32 |  |
| nh_ap_hwn | int32 |  |
| nh_np_hwn | int32 |  |
| nh_ap_oth | int32 |  |
| nh_np_oth | int32 |  |
| nh_1_race | int32 |  |
| nh_2_races | int32 |  |
| nh_2_race1 | int32 |  |
| nh_3_races | int32 |  |
| nh_4_races | int32 |  |
| nh_5_races | int32 |  |
| nh_omb_oth | int32 |  |
| f18_whtblk | int32 |  |
| f18_whtind | int32 |  |
| f18_whtasn | int32 |  |
| f18_whthwn | int32 |  |
| f18_whtoth | int32 |  |
| f18_blkind | int32 |  |
| f18_blkasn | int32 |  |
| f18_blkhwn | int32 |  |
| f18_blkoth | int32 |  |
| f18_indasn | int32 |  |
| f18_indhwn | int32 |  |
| f18_indoth | int32 |  |
| f18_asnhwn | int32 |  |
| f18_asnoth | int32 |  |
| f18_hwnoth | int32 |  |
| f18_whtbl1 | int32 |  |
| f18_whtbl2 | int32 |  |
| f18_whtbl3 | int32 |  |
| f18_whtbl4 | int32 |  |
| f18_whtin1 | int32 |  |
| f18_whtin2 | int32 |  |
| f18_whtin3 | int32 |  |
| f18_whtas1 | int32 |  |
| f18_whtas2 | int32 |  |
| f18_whthw1 | int32 |  |
| f18_blkin1 | int32 |  |
| f18_blkin2 | int32 |  |
| f18_blkin3 | int32 |  |
| f18_blkas1 | int32 |  |
| f18_blkas2 | int32 |  |
| f18_blkhw1 | int32 |  |
| f18_indas1 | int32 |  |
| f18_indas2 | int32 |  |
| f18_indhw1 | int32 |  |
| f18_asnhw1 | int32 |  |
| f18_whtbl5 | int32 |  |
| f18_whtbl6 | int32 |  |
| f18_whtbl7 | int32 |  |
| f18_whtbl8 | int32 |  |
| f18_whtbl9 | int32 |  |
| f18_whtb10 | int32 |  |
| f18_whtin4 | int32 |  |
| f18_whtin5 | int32 |  |
| f18_whtin6 | int32 |  |
| f18_whtas3 | int32 |  |
| f18_blkin4 | int32 |  |
| f18_blkin5 | int32 |  |
| f18_blkin6 | int32 |  |
| f18_blkas3 | int32 |  |
| f18_indas3 | int32 |  |
| f18_whtb11 | int32 |  |
| f18_whtb12 | int32 |  |
| f18_whtb13 | int32 |  |
| f18_whtb14 | int32 |  |
| f18_whtin7 | int32 |  |
| f18_blkin7 | int32 |  |
| f18_all_ra | int32 |  |
| f18_ap_wht | int32 |  |
| f18_np_wht | int32 |  |
| f18_np_asn | int32 |  |
| f18_np_blk | int32 |  |
| f18_np_ind | int32 |  |
| f18_ap_blk | int32 |  |
| f18_ap_ind | int32 |  |
| f18_ap_asn | int32 |  |
| f18_ap_hwn | int32 |  |
| f18_np_hwn | int32 |  |
| f18_ap_oth | int32 |  |
| SHAPE__Length | double |  |
| SHAPE__Area | double |  |
| geometry | binary |  |
| geometry_bbox | struct<xmin: float not null, ymin: float not null, xmax: float not null, ymax: float not null> |  |

## Files

| File | Size | Checksum |
|------|------|----------|
| ./wards.parquet | 161.9 KB | 1220489c01b0... |
| ./wards.pmtiles | 483.0 KB | 1220c1ec8fb0... |
| ./styles/city-renderer.json | 725 B | 122012cb82b8... |
| ./styles/default.json | 2.8 KB | 12208fa26bfc... |
| ./styles/style-boundaries.json | 452 B | 12207f5d8a7a... |
| ./styles/style-subtle.json | 709 B | 122039df8a1c... |
| ./thumbnail.png | 326.7 KB | 12205e421b44... |
| https://static.stlouis-mo.gov/open-data/planning/wards/wards_2020.zip | 78.0 KB | 1220d5ef9b56... |

## Quick Start

```python
import geopandas as gpd

gdf = gpd.read_parquet("./wards.parquet")
print(gdf.head())
```

## STAC Metadata

- **root**: `../../catalog.json`
- **parent**: `../catalog.json`

## Source

[https://static.stlouis-mo.gov/open-data/planning/wards/wards_2020.zip](https://static.stlouis-mo.gov/open-data/planning/wards/wards_2020.zip)

## Processing Notes

Mirrored from the City of St. Louis open data portal (https://www.stlouis-mo.gov/data/datasets/dataset.cfm?id=131). Nothing was added to the data and no features were dropped except where noted below.

Downloaded from the city's portal: https://static.stlouis-mo.gov/open-data/planning/wards/wards_2020.zip (zipped Shapefile).

The city's Hosted ward FeatureServer carries only 10 of the 14 wards, so this collection is built from the portal's shapefile download, which has all 14.

Converted to GeoParquet with gpio — zstd compression, Hilbert row order, and a covering bbox column with row-group statistics, so a spatial filter can skip most of the file over the network — and tiled to PMTiles with tippecanoe.

The city's own file(s) are published as `source` assets on this collection, linked directly to stlouis-mo.gov — this mirror never becomes the only way to reach the original.


## Attribution

City of St. Louis — Planning and Urban Design

## License

[other](https://www.stlouis-mo.gov/data/datasets/dataset.cfm?id=131)

## Contact

Chris Holmes <cholmes@9eo.org>

---

*Generated by [Portolan](https://github.com/portolan-sdi/portolan-cli) from STAC metadata and .portolan/metadata.yaml*
